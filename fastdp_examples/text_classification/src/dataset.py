...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        if args.part_id is not None:
            part_size = len(self.support_examples) // 5
            self.support_examples = self.support_examples[part_size * args.part_id: part_size * (args.part_id + 1)]
            part_size = len(self.query_examples) // 5
            self.query_examples = self.query_examples[:part_size]

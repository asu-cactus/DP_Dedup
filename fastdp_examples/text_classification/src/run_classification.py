...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    part_id: int = field(
        default=4,
        metadata={"help": "Partition ID for training"},
    )
...
def main():
    ...
    if model_args.few_shot_type == "finetune":
        ...
        train_dataset = FewShotDataset(
            data_args, tokenizer=tokenizer, mode="train", use_demo=use_demo
        )
        eval_dataset = (
            FewShotDataset(
        ...
        test_dataset = (
            FewShotDataset(
                data_args, tokenizer=tokenizer, mode="test", use_demo=use_demo
            )
            if training_args.do_predict or training_args.evaluate_test_split
            else None
        )
    ...
    for _tag, _ds in zip(
        ("train", "valid", "test"), (train_dataset, eval_dataset, test_dataset)
    ):
        if _ds is not None:
    ...
    print(
        f"bias_only: {model_args.bias_only} | attention_only: {model_args.attention_only} | randomly_initialize: {model_args.randomly_initialize}"
    )
    ...
    print(f"Number of trainable params: {num_differentiable_params / 1e6:.3f} million")
    print(
        f"Number of total params: {utils.count_parameters(model, only_differentiable=False) / 1e6:.3f} million"
    )
    ...
    if privacy_args.non_private:
        ...
        origin_params = (
            None
            if model_args.bias_only or model_args.attention_only
            else ["_embeddings"]
        )
    ...
    if training_args.do_eval:
        ...
        for split, dataset in zip(("dev", "test"), (eval_dataset, test_dataset)):
            ...
            if data_args.task_name == "mnli":
                ...
                eval_splits.append(split)
                if model_args.few_shot_type == "finetune":
                    mnli_mm_dataset = GlueDataset(
                        mnli_mm_data_args, tokenizer, mode=split
                    )
        ...
        for eval_dataset, eval_task_name, eval_split in zip(
            eval_datasets, eval_task_names, eval_splits
        ):
            ...
            if eval_result is not None:
                if not privacy_args.non_private:
                    privacy_spent = privacy_engine.get_privacy_spent(
                        accounting_mode="all", lenient=True
                    )
            ...
            if trainer.is_world_process_zero():
                with open(output_test_file, "w") as writer:
                    ...
                    for key, value in test_result.items():
                        ...
                        writer.write("%s = %s\n" % (key, value))
                        final_result[test_dataset.args.task_name + "_test_" + key] = (
                            value
                        )

...
class Trainer(transformers.Trainer):
    ...
    def get_training_setup(self):
        ...
        print(f"train_dataloader batch size: {train_dataloader.batch_size}")
    ...
    def train(self, model_path=None, dev_objective=None, dev_objective_key=None):
        ...
        if self.args.local_rank != -1 or self.args.n_gpu > 1:
            logger.info(f"local_rank: {self.args.local_rank}, n_gpu: {self.args.n_gpu}")
            raise ValueError(
        ...
        self.dev_objective = (
            default_dev_objective if dev_objective is None else dev_objective
        )
        self.dev_objective_key = (
            default_dev_objective_key
            if dev_objective_key is None
            else dev_objective_key
        )
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (
                        ...
                        in (IntervalStrategy.STEPS, EvaluationStrategy.STEPS)
            ...
            if (
                self.args.evaluation_strategy == IntervalStrategy.EPOCH
        ...
        return (
            TrainOutput(self.global_step, tr_loss / self.global_step, metrics=metrics),
            self.objective,
        )
    ...
    def evaluate_and_log(
        ...
        # Write to disk!
        utils.jdump(
            self.log_history, os.path.join(self.args.output_dir, "log_history.json")
        )
        # ---
